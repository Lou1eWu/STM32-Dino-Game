...
// 恐龙游戏主文件中添加以下内容
#include "Key.h" // 包含新的按键处理头文件
// 全局变量定义（添加到原有全局变量中）
uint8_t keyEvent = 0; // 记录当前按键事件
uint8_t doubleClickFlag = 0; // 双击标志
...
//			Num1 = 0;
//		}
//		OLED_ShowNum(1, 6, Num1, 5, OLED_8X16);
//		OLED_ShowNum(1, 32, Num2, 5, OLED_8X16);
//		OLED_Update();
//		OLED_Clear();
	}
}
...
// 长按释放时取消无敌状态
		if (Key_Check(KEY_2, KEY_UP) && longPressFlag)
			{
				longPressFlag = 0;
			}
}
...
//	}
		...
		/* down run status */
	if (doubleClickFlag == 1 && dino_jump_finish_flag != 1)
		{
			doubleClickFlag = 0; // 重置双击标志
			OLED_ClearArea(16, 42, 15, 16); // 清除奔跑状态恐龙
			barrier_flag_num = pos;
		if(barrier_flag_num >128)
		{	
			barrier_flag_num = 0;
